"""EA-0012 adapter for SaaS-scoped baseline assessment."""
...
class _SelectedBaselineStore:
    ...
    async def put(self, baseline: Baseline) -> Baseline:
        if baseline.id not in self.selected_ids:
            raise SaaSConfigInvalid("cannot add an unselected SaaS baseline")
...
class AssetConfigSaaSBaselineRouter:
    """Delegate configured SaaS baseline assessment to EA-0012."""
        ...
        *,
        ...
        for baseline_id in sorted(selected_ids):
            ...
            if baseline is None or not _tenant_visible(baseline.tenant_id, tenant_id):
                raise SaaSConfigInvalid(f"SaaS baseline unavailable: {baseline_id!r}")
        ...
        if labels is not None and not isinstance(labels, Mapping):
            raise SaaSConfigInvalid("SaaS baseline scope labels must be a mapping")
        selected_labels = {} if labels is None else dict(labels)
        if selected_labels.get("module") not in (None, "EA-0029"):
            raise SaaSConfigInvalid("SaaS baseline scope cannot select another module")
        selected_labels["module"] = "EA-0029"
