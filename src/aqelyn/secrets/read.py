"""Read-only cryptographic asset projection owned by the secrets package."""
...
from aqelyn.secrets.models import CryptoAsset, CryptoAssetKind
from aqelyn.secrets.store import CryptoStore

_CURSOR_NAMESPACE = "crypto-asset-v1"
_HEALTH_TENANT = "00000000-0000-0000-0000-000000000000"


class SecretsReadService:
    """Expose value-free asset metadata and the owner's lifecycle explanation."""
    ...
    def __init__(
        ...
        *,
        ...
        dependencies: Sequence[str] = ("secrets_engine",),
        critical: bool = False,
    ) -> None:
        self._store = store
        self._engine = engine
    ...
    async def list_assets(
        ...
        *,
        ...
        limit: int,
        cursor: str | None,
    ) -> ReadPage[CryptoAsset]:
        after: tuple[CryptoAssetKind, str] | None = None
        namespace = _cursor_namespace(tenant_id)
        if cursor is not None:
            kind, asset_id = decode_keyset_cursor(
                cursor,
                namespace=namespace,
                arity=2,
            )
            after = cast(CryptoAssetKind, kind), asset_id
        records, next_key = await self._store.query_assets_for_read(
            tenant_id=tenant_id,
            after=after,
            limit=limit,
        )
        return ReadPage(
            items=tuple(
                ReadItem(record=record, explain=self._engine.explain(record)) for record in records
            ),
            next_cursor=(
                None
                if next_key is None
    ...
    async def get_asset(
        ...
        *,
        tenant_id: str | None,
    ) -> ReadItem[CryptoAsset] | None:
        record = await self._store.get_asset(asset_id, tenant_id=tenant_id)
        if record is None:
            return None
        return ReadItem(record=record, explain=self._engine.explain(record))

    async def _check_store(self) -> None:
        tenant_id = _HEALTH_TENANT if self._tenant_mode == "enterprise" else None
        try:
            await self._store.query_assets_for_read(tenant_id=tenant_id, limit=1)
        except Exception as exc:
            raise StoreUnavailable(f"secrets read store unavailable: {exc}") from exc
