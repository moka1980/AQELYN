...
async def test_identity_detection_query_ordered_prefixes(
    ...
    ids = sorted(new_id("idt") for _ in range(ROW_COUNT))
    # Leading-key groups descend across ascending IDs; IDs still decide each tie.
    records = [
        _detection(
            detection_id=row_id,
            detected_at=BASE + timedelta(minutes=((ROW_COUNT // 2) - 1) - (index // 2)),
        )
        for index, row_id in enumerate(ids)
    ]
    ordered = sorted(records, key=lambda row: (row.detected_at, row.id))
