...
def _expected_exempt_fields() -> dict[str, str]:
    groups: tuple[tuple[str, tuple[str, ...], str], ...] = (
        ...
        (
            "identity",
            ("redeemed_by",),
            "ECR-0116 keeps single-use invite bookkeeping inside the identity owner: redeemed_by "
            "is read only within the invite store to refuse reuse, and no external consumer "
            "references it. (email became consumed once ECR-0118's portal login/register reads "
            "it.)",
        ),
        (
            "consent",
            ("revoked_at",),
            "ECR-0117 keeps the consent revocation timestamp inside its owner: external "
            "consumers receive ConsentRecord envelopes through active() and never read "
            "revoked_at directly. (actor_account_id and text_version became consumed once "
            "ECR-0118's portal writes them.)",
        ),
    )
