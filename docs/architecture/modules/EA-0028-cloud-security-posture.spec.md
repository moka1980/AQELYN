...
cloud `Baseline`s over cloud-scoped assets — no drift logic here (D4). If the
caller supplies no `limit`, the router must not materialize `ObjectQuery`'s
default limit as an invisible cloud-estate bound; the delegated assessment pages
to exhaustion and records no synthetic `limit` in the snapshot scope (ECR-0029).
...
- **FR-7** Cloud config assessment SHALL be performed by **EA-0012** using cloud `Baseline`s; the module SHALL NOT implement drift detection (D4). EA-0012 SHALL be able to assess cloud object types through the configuration used by both shipped runtime factories; CSPM SHALL NOT relabel cloud resources as `"asset"` to reach it. An assessment that applied **no baseline** to in-scope objects SHALL be surfaced, never returned as a zero-drift snapshot, and partial/truncated baseline coverage SHALL remain visible in the snapshot — not assessed ≠ compliant. A no-scope `apply_cloud_baselines` call SHALL assess the whole cloud estate rather than a default-size prefix (**ECR-0027/ECR-0028/ECR-0029**).
