...
independent CI green on `main`. Correction to the session record: CI runs for #318–#324 DID exist —
each failed in under a minute at the ruff format gate (unpinned `ruff>=0.5`; CI resolved 0.16.1,
local ran 0.15.21), so mypy and pytest never executed in CI and every merge in that line is on
verified local green only. The pin to 0.16.2 travels with ECR-0123's PR. The review's two P1 blockers are already closed in the repo:
